...
void QViewer::updateView(AVFrame* pFrame)
{
  int bits = 0;
  char* pBits = (char*)qDisplayImage.bits();
  for(uint32_t i = 0; i < resolution.height(); ++i) {
    memcpy(pBits + bits, (char*)pFrame->data[0] + i * pFrame->linesize[0], resolution.width() * 3);
    bits += resolution.width() * 3;
  }
  setPixmap(QPixmap::fromImage(qDisplayImage, Qt::AutoColor));
}
